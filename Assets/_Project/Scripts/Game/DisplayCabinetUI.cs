...
{
    // 食器棚の暗証番号入力UI
    public class DisplayCabinetUI : MonoBehaviour
    {
        [SerializeField] private GameObject panel;
        [SerializeField] private TextMeshProUGUI codeDisplay;

        private DisplayCabinetPuzzle puzzle;
        private bool solved;

        private void Awake()
        {
            puzzle = FindAnyObjectByType<DisplayCabinetPuzzle>();
            if (puzzle != null) puzzle.OnSolved.AddListener(OnSolved);
            else Debug.LogError("[DisplayCabinetUI] DisplayCabinetPuzzleが見つかりません");
        ...
        private void Update()
        {
            ...
            if (panel.activeSelf && RoomViewController.Instance.CurrentArea != RoomArea.DisplayCabinet)
